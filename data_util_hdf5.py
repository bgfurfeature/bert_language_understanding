...
def load_data_multilabel(data_path, traning_data_path, valid_data_path, test_data_path, vocab_word2index, label2index,
                         sentence_len, process_num=20, test_mode=False, tokenize_style='word', model_name=None):
    """
    ...
    t1 = time.clock()
    print(
        "###load_data_multilabel.data_path:", data_path, ";traning_data_path:", traning_data_path, ";valid_data_path:",
        valid_data_path, ";test_data_path:", test_data_path)
    print(
        "###vocab_word2index:", len(vocab_word2index), ";label2index:", len(label2index), ";sentence_len:",
        sentence_len)
    ...
    print("cache_path:", cache_file, "train_valid_test_file_exists:", cache_file_exist_flag, ";traning_data_path:",
          traning_data_path, ";valid_data_path:", valid_data_path)
    ...
    print(
        "load_data_multilabel.length of train_lines:", number_examples, ";valid_lines:", len(valid_lines),
        ";test_lines:",
        len(test_lines))
    ...
    for chunk_id, each_chunk in enumerate(chunks):
        ...
        pool.apply_async(transform_data_to_index, args=(
            each_chunk, file_name, vocab_word2index, label2index, sentence_len, 'train', tokenize_style))
    pool.close()
    pool.join()
    ...
    valid = transform_data_to_index(valid_lines, None, vocab_word2index, label2index, sentence_len, 'valid',
                                    tokenize_style)
    test = transform_data_to_index(test_lines, None, vocab_word2index, label2index, sentence_len, 'test',
                                   tokenize_style)

    # 4. save to file system if data not exists
    if not os.path.exists(cache_file):
        print(
            "going to dump train/valid/test data to file sytem!")  # pickle.dump((train,valid,test),data_f,protocol=pickle.HIGHEST_PROTOCOL) #TEMP REMOVED. ,protocol=2
...
def transform_data_to_index(lines, target_file_path, vocab_word2index, label2index, sentence_len, data_type,
                            tokenize_style):
    """
    ...
    for i, line in enumerate(lines):
        try:
            ...
            if i % 100 == 0:
                ...
                print(
                    data_type, i, "transform_data_to_index.input_labels:", input_labels, ";label_list:", label_list,
                    ";y:",
                    y)
        except Exception as e:
            if random.randint(0, 10) == 1: print(
                "ignore line. you may be in test_model=True, label may not exist.", line, e)
    X = np.array(X)
    Y = np.array(Y)
...
def create_raw_data(data_path,raw_data_path):
    file_object = codecs.open(raw_data_path, mode='r', encoding='utf-8')
    lines = file_object.readlines()
    file_object.close()
    ...
    with open(data_path + "word2vec/bert_raw_without_label.txt", 'w') as f:
        for i,line in enumerate(lines):
...
def create_or_load_vocabulary(data_path, training_data_path, vocab_size, test_mode=False, tokenize_style='word',
                              fine_tuning_stage=False, model_name=None):
    """
    ...
    """
    print("create_or_load_vocabulary.data_path:", data_path, ";training_data_path:", training_data_path, ";vocab_size:",
          vocab_size, ";test_mode:", test_mode, ";tokenize_style:", tokenize_style)
...
def get_lable2index(data_path, training_data_path, tokenize_style='word'):
    ...
    for i, line in enumerate(lines):
        ...
        c_labels.update(input_label)
        if i % 1000 == 0:  # print some information for debug purpose
            print(i, "get_lable2index.line:", line);
            print(i, "get_lable2index.input_label:", input_label)
    label2index = {}
    label_list = c_labels.most_common()
    for i, tuplee in enumerate(label_list):
        label_name, freq = tuplee
...
data_path = './data/'
# data for train word2vec
bert_raw_data = data_path + 'bert_raw.txt'
traning_data_path = data_path + 'bert_train.txt'
valid_data_path = data_path + 'bert_valid.txt'
test_data_path = data_path + 'bert_test.txt'
vocab_size = 50000
process_num = 5
test_mode = True
sentence_len = 200
