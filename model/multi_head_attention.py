...
class MultiHeadAttention(object):
    ...
    def __init__(self, Q, K_s, V_s, d_model, d_k, d_v, sequence_length, h, typee=None, is_training=None, mask=None,
                 dropout_rate=0.1):
    ...
    def multi_head_attention_fn(self):
        ...
        dot_product = self.scaled_dot_product_attention_batch(Q_projected, K_s_projected,
                                                              V_s_projected)  # [batch,h,sequence_length,d_v]
        # 3. concatenated
    ...
    def scaled_dot_product_attention_batch_mine(self, Q, K_s,
                                                V_s):  # my own implementation of scaled dot product attention.
        """
    ...
    def scaled_dot_product_attention_batch(self, Q, K_s,
        ...
        Q_heads = tf.stack(tf.split(Q, self.h, axis=2), axis=1)  # [batch,h,sequence_length,d_k]
        K_heads = tf.stack(tf.split(K_s, self.h, axis=2), axis=1)  # [batch,h,sequence_length,d_k]
        V_heads = tf.stack(tf.split(V_s, self.h, axis=2),
                           axis=1)  # [batch,h,sequence_length,d_v]. during implementation, d_v=d_k.
        ...
        # mask: 要求当前位置看不到sequence_length后面的其他位置的下文信息
        if self.mask is not None:
            mask_expand = tf.expand_dims(tf.expand_dims(self.mask, axis=0),
                                         axis=0)  # [1,1,sequence_length,sequence_length]
        ...
        # tf.reduce_sum(tf.exp(logits)) 得到是一个sum数值
